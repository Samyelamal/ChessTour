...
def display_round(round_) -> None:
    ...
    for i, match in enumerate(round_.matches, 1):
        p1 = match.player_1
        p2 = match.player_2
        print(f"  Match {i} : {p1.first_name} {p1.last_name} vs "
...
def display_tournament_players(players) -> None:
    ...
    print("\n=== JOUEURS DU TOURNOI ===")
    for p in sorted(players, key=lambda x: (x["last_name"], x["first_name"])):
...
def display_tournament_rounds(rounds) -> None:
    ...
    for round_ in rounds:
        ...
        for i, match in enumerate(round_["matches"], 1):
            ...
            p2 = match["player_2"]
            print(
                f"    Match {i} : {p1['first_name']} {p1['last_name']} ({match['score_1']}) "
                f"vs {p2['first_name']} {p2['last_name']} ({match['score_2']})"
            )
...
def prompt_new_tournament() -> tuple:
    ...
    number_of_rounds = int(rounds_input) if rounds_input.isdigit() else 4
    description = input("Description : ").strip()
    return name, location, start_date, end_date, number_of_rounds, description


def prompt_new_player() -> tuple:
    """Demande à l'utilisateur les informations pour ajouter un joueur."""
    print("\n--- Ajouter un joueur ---")
...
def prompt_match_result(match, index: int) -> tuple:
    """Demande à l'utilisateur le résultat d'un match et retourne les scores."""
    p1 = match.player_1
    p2 = match.player_2
    print(f"\n  Match {index} : {p1.first_name} {p1.last_name} vs "
          f"{p2.first_name} {p2.last_name}")
